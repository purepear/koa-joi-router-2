...
exports.routeToSwaggerPaths = function routeToSwaggerPaths(route, userOptions, json_schemas) {
	...
	const separatorIndex = route.name.lastIndexOf('_');
	const parent = route.name.substring(0, separatorIndex);
	const actualRoute = route.name.substring(separatorIndex+1);
...
exports.swaggerizePath = function swaggerizePath(path) {
	...
	const segments = pathTokens.map(function (token) {
		...
		if (token.name) {
			...
			while (token[0]=== '/') {
				token = token.substring(1); // remove leading slash, to handle things like complex routes: /users/:userId/friends/:friendId
			}
			segment = token;
		}
